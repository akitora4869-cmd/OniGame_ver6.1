...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    private NamespacedKey actionKey, equipmentUsesKey;
    ...
    private void reportError(String context, Throwable error){
        ...
        for(Player viewer:Bukkit.getOnlinePlayers())if(viewer.isOp())viewer.sendMessage(chat);
    }
    ...
    private boolean isOniTypeLocked(OniType type){return type!=null&&getConfig().getBoolean("content-locks."+oniLockKey(type),false);}
    private OniType firstUnlockedOni(){for(OniType t:new OniType[]{OniType.DAKKO,OniType.KISHIN,OniType.SHIKKI,OniType.YUUKI,OniType.KANKI,OniType.JAKUTSUKI})if(!isOniTypeLocked(t))return t;return OniType.DAKKO;}
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("arenasetup")&&sender instanceof Player p){
            ...
            if(a.length>1&&a[1].equalsIgnoreCase("info")){
                ...
                msg(p,"通常アリーナ: "+(getConfig().getBoolean("arena-setup.enabled",false)?"&a有効":"&c無効")
                    ...
                    +" &7/ 心臓候補: &e"+hearts+" &7/ 出口: &e"+(exit1!=null?1:0)+"/2"+(exit2!=null?" &a設定済み":" &c出口B未設定")+" &7/ ボーダー: &e"+getConfig().getInt("arena-setup.size",200)+"x"+getConfig().getInt("arena-setup.size",200));
                return true;
            }
        ...
        if(a[0].equalsIgnoreCase("lobbyarea")&&sender instanceof Player p){
            ...
            if(sub.equals("info")){Location la=LocationStore.get(getConfig(),"locations.lobby-area-pos1"),lb=LocationStore.get(getConfig(),"locations.lobby-area-pos2");msg(p,"ロビー境界: "+(la!=null&&lb!=null?"&a設定済み":"&c未設定")+" &7/ map_makerタグは境界TPを回避");return true;}
        ...
        if(a[0].equalsIgnoreCase("mode")){if(a.length<2){String cm=getConfig().getString("game-mode","normal").toLowerCase(Locale.ROOT);msg(sender,"現在のモード: &e"+(cm.equals("duo")?"二人鬼":cm.equals("escape")?"ESCAPE（大人数脱出）":"通常"));return true;}if(state!=GameState.WAITING){msg(sender,"&cゲーム中はモードを変更できません。");return true;}String m=a[1].toLowerCase(Locale.ROOT);if(!List.of("normal","duo","escape").contains(m)){msg(sender,"/og mode <normal|duo|escape>");return true;}getConfig().set("game-mode",m);saveConfig();all("&eゲームモードを &f"+(m.equals("duo")?"二人鬼（鬼2 / ぷれいやー12）":m.equals("escape")?"ESCAPE（広域イベント脱出）":"通常")+" &eに変更しました。");return true;}
    ...
    private boolean handleAreaCommand(CommandSender sender,String[] a){
        ...
        if(!p.hasPermission("onigame.admin")){msg(p,"&c権限がありません。");return true;}
    ...
    private void removePlayerBots(){configuredFriendlyNpcId=null;eliteBotMatchIds.clear();for(UUID id:new HashSet<>(playerBots)){Entity entity=Bukkit.getEntity(id);if(entity!=null)entity.remove();}azakujiAllyId=null;azakujiFightUntil=0L;azakujiEscapeUntil=0L;azakujiTauntUntil=0L;azakujiNextLeapAt=0L;azakujiNextMeleeAt=0L;azakujiLastCombatAt=0L;playerBots.clear();escapedPlayerBots.clear();deadPlayerBots.clear();playerBotSkills.clear();playerBotPassives.clear();playerBotPresetNames.clear();playerBotSkillReadyAt.clear();playerBotSprintUntil.clear();playerBotStamina.clear();playerBotTargetHeart.clear();playerBotLastLocation.clear();playerBotStuckTicks.clear();}
    ...
    private ItemStack playerSkillMenuItem(PlayerSkill skill,PlayerSkill current){
        String[] lore=switch(skill){
            ...
            case HEAL->new String[]{"&7回復ゲージ完了後に体力8回復","&7絆なしでは自分だけを回復","&7攻撃・移動で中断 / CT: 45秒"};
    ...
    private void addPlayerBuildFooter(Player p,Inventory menu,boolean passiveTab,int page,int maxPage){
        ...
        menu.setItem(50,menuItem(Material.BOOK,"&6&l役割について","menu_decor","&7能力を用途ごとに分類しています。","&7同じ役割の中から比較しやすくなっています。"));
    ...
    private ItemStack oniMask(int model,String name){
        ItemStack i=item(Material.CARVED_PUMPKIN,name,"oni_costume");ItemMeta m=i.getItemMeta();m.setCustomModelData(model);m.setUnbreakable(true);m.setLore(List.of(cc("&8鬼専用外見")));i.setItemMeta(m);return i;
    }
    private ItemStack dyedOniArmor(Material material,org.bukkit.Color color,String name){
        return dyedOniArmor(material,color,name,null);
    }
    private ItemStack dyedOniArmor(Material material,org.bukkit.Color color,String name,Integer customModelData){
        ItemStack i=item(material,name,"oni_costume");ItemMeta meta=i.getItemMeta();meta.setUnbreakable(true);meta.setLore(List.of(cc("&8鬼専用外見")));
        if(customModelData!=null)meta.setCustomModelData(customModelData);
        if(meta instanceof LeatherArmorMeta leather)leather.setColor(color);i.setItemMeta(meta);return i;
    }
    ...
    private void equipShikkiArmor(Player p){p.getInventory().setHelmet(oniArmorWithModel(Material.DIAMOND_HELMET,"&c疾鬼の兜",1210));p.getInventory().setChestplate(oniArmorWithModel(Material.DIAMOND_CHESTPLATE,"&4疾鬼の装束",1211));p.getInventory().setLeggings(oniArmorWithModel(Material.DIAMOND_LEGGINGS,"&8疾鬼の袴",1212));p.getInventory().setBoots(oniArmorWithModel(Material.DIAMOND_BOOTS,"&4疾鬼の足具",1213));}
    private void equipYuukiArmor(Player p){p.getInventory().setHelmet(dyedOniArmor(Material.GOLDEN_HELMET,org.bukkit.Color.fromRGB(35,22,48),"&5幽鬼の兜",1220));p.getInventory().setChestplate(dyedOniArmor(Material.GOLDEN_CHESTPLATE,org.bukkit.Color.fromRGB(35,22,48),"&5幽鬼の装束",1221));p.getInventory().setLeggings(dyedOniArmor(Material.GOLDEN_LEGGINGS,org.bukkit.Color.fromRGB(18,17,24),"&8幽鬼の袴",1222));p.getInventory().setBoots(dyedOniArmor(Material.GOLDEN_BOOTS,org.bukkit.Color.fromRGB(52,31,67),"&5幽鬼の足具",1223));}
    private void equipShikkiArmor(LivingEntity e){e.getEquipment().setHelmet(oniArmorWithModel(Material.DIAMOND_HELMET,"&c疾鬼の兜",1210));e.getEquipment().setChestplate(oniArmorWithModel(Material.DIAMOND_CHESTPLATE,"&4疾鬼の装束",1211));e.getEquipment().setLeggings(oniArmorWithModel(Material.DIAMOND_LEGGINGS,"&8疾鬼の袴",1212));e.getEquipment().setBoots(oniArmorWithModel(Material.DIAMOND_BOOTS,"&4疾鬼の足具",1213));}
    private void equipYuukiArmor(LivingEntity e){e.getEquipment().setHelmet(dyedOniArmor(Material.GOLDEN_HELMET,org.bukkit.Color.fromRGB(35,22,48),"&5幽鬼の兜",1220));e.getEquipment().setChestplate(dyedOniArmor(Material.GOLDEN_CHESTPLATE,org.bukkit.Color.fromRGB(35,22,48),"&5幽鬼の装束",1221));e.getEquipment().setLeggings(dyedOniArmor(Material.GOLDEN_LEGGINGS,org.bukkit.Color.fromRGB(18,17,24),"&8幽鬼の袴",1222));e.getEquipment().setBoots(dyedOniArmor(Material.GOLDEN_BOOTS,org.bukkit.Color.fromRGB(52,31,67),"&5幽鬼の足具",1223));}
    ...
    private TestArenaLayout createRandomTestArena(Location center){
        ...
        Location player=randomTestSurface(center,random,half,occupied,0);occupied.add(player);
        Location oni=randomTestSurfaceFarFrom(center,random,half,occupied,Math.min(70,size*.35));occupied.add(oni);
        Location exit=randomTestSurfaceFarFrom(center,random,half,occupied,Math.min(38,size*.19));occupied.add(exit);
        Location exit2=randomTestSurfaceFarFrom(center,random,half,occupied,Math.min(55,size*.275));occupied.add(exit2);
        List<Location> hearts=new ArrayList<>();
    ...
    private void registerUnstuckAbuseAttempt(Player p,String reason){
        ...
        if(getConfig().getBoolean("safety-rescue.notify-ops-on-lock",true)){
            for(Player op:Bukkit.getOnlinePlayers())if(op.isOp())msg(op,"&e"+p.getName()+" &7の /og unstuck を悪用疑いで一時ロックしました。");
        }
    }
    ...
    private void updateOniBot(){
        ...
        if(oniType==OniType.DAKKO){
            ...
            else{useBotShikkiLeapChain(bot,target);botNextAbilityAt=now+oniBotAbilityCooldownMillis(getConfig().getInt("shikki-skills.hunting-leap.bot-cooldown-seconds",6));}
    ...
    private void useBotShikkiLeapChain(Mob bot,LivingEntity initialTarget){
        int jumps=botDifficulty==BotDifficulty.NORMAL?1:(botDifficulty==BotDifficulty.ELITE?2:3);
    ...
    private void botShikkiLeapStep(Mob bot,LivingEntity target,int step,int max){
        ...
        Vector leap=aim.toVector().subtract(bot.getLocation().toVector());if(leap.lengthSquared()<.01)return;leap.normalize().multiply(getConfig().getDouble("shikki-skills.hunting-leap.bot-power",1.45));leap.setY(Math.max(.48,leap.getY()+.48));bot.setVelocity(leap);bot.setFallDistance(0);bot.getWorld().spawnParticle(Particle.CLOUD,bot.getLocation(),18,.3,.15,.3,.05);bot.getWorld().playSound(bot.getLocation(),Sound.ENTITY_ENDER_DRAGON_FLAP,.7f,(float)(1.4+step*.1));
        if(step>=max)return;
        long delay=step==1?Math.max(2,getConfig().getLong("shikki-skills.hunting-leap.bot-chain-delay-1-ticks",10)):Math.max(2,getConfig().getLong("shikki-skills.hunting-leap.bot-chain-delay-2-ticks",6));
        ...
        Bukkit.getScheduler().runTaskLater(this,()->{
            ...
            task[0]=Bukkit.getScheduler().runTaskTimer(this,()->{
                ...
                if(distance<=getConfig().getDouble("skills.bugei.impact-distance",2.2)){
                    double damage=heartGoalReached()?getConfig().getDouble("skills.bugei.final-damage",18.0):getConfig().getDouble("skills.bugei.sealed-damage",10.0);
    ...
    private void updateHorrorState(){
        ...
        for(UUID id:players){
            ...
            else if(chased.contains(id)&&now-lastSeenAt.getOrDefault(id,now)>lostMillis){chased.remove(id);onExpandedOniChaseLost(target,now);scheduleLeaveChaseBgm(target);target.sendTitle(cc("&7追跡から逃れた"),cc("&8息を潜めろ……"),5,30,10);target.playSound(target.getLocation(),Sound.BLOCK_AMETHYST_BLOCK_RESONATE,0.5f,0.6f);}
        }
    ...
    private long oniBotAbilityCooldownMillis(double seconds){
        double multiplier=hasOniPassive(OniPassiveSkill.MASTERY)?Math.max(.50,Math.min(1.0,getConfig().getDouble("oni-passive-skills.mastery-cooldown-multiplier",.85))):1.0;LivingEntity hunter=getOniEntity();if(hasOniPassive(OniPassiveSkill.TERRITORY)&&hunter!=null&&oniNearAliveHeart(hunter,getConfig().getDouble("oni-passive-skills.territory-radius",8.0)))multiplier*=Math.max(.50,Math.min(1.0,getConfig().getDouble("oni-passive-skills.territory-cooldown-multiplier",.80)));return Math.max(1000L,(long)(seconds*1000.0*multiplier));
    }
    ...
    private void enforceLobbyBoundary(Player p,Location to){if(state!=GameState.WAITING||p==null||p.isOp()&&p.getScoreboardTags().contains("map_maker")||p.getScoreboardTags().contains("map_maker")||isInTrainingArea(to)||isInLobbyArea(to))return;Location lobby=LocationStore.get(getConfig(),"locations.lobby");if(lobby==null||lobby.getWorld()==null)return;p.sendTitle(cc("&c&lロビーエリア外です"),cc("&7ロビーへ戻ります。 &8(map_makerタグで回避可能)"),0,30,10);p.teleport(lobby);p.setFallDistance(0);}
    ...
    private void enterTraining(Player p){if(!trainingPlayers.add(p.getUniqueId()))return;applyTrainingPassives(p);giveTrainingItems(p);p.sendTitle(cc("&b&lTRAINING MODE"),cc("&fスキル＋パッシブ実戦テスト"),5,35,10);p.sendActionBar(cc("&b練習モード &7――選択中のぷれいやー/鬼パッシブ有効・鬼スキル全解放"));Location dl=LocationStore.get(getConfig(),"locations.training-dummy");if(dl!=null&&(trainingDummy==null||Bukkit.getEntity(trainingDummy)==null))spawnTrainingDummy(dl);}
    private void leaveTraining(Player p){trainingPlayers.remove(p.getUniqueId());removeTrainingItems(p);cooldowns.keySet().removeIf(k->k.startsWith(p.getUniqueId()+":"));resetTrainingPassiveStats(p);p.sendActionBar(cc("&7練習モードを終了しました。"));}
    ...
    private boolean ready(Player p,String id,int cd){if(players.contains(p.getUniqueId())&&System.currentTimeMillis()<oniSkillSealUntil.getOrDefault(p.getUniqueId(),0L)){msg(p,"&5封殺 &7――スキルが封じられている。");return false;}boolean training=state==GameState.WAITING&&isInTrainingArea(p.getLocation());if(training)cd=Math.max(1,getConfig().getInt("training.cooldown-seconds",1));boolean oniSkill=isOniSkillAction(id)&&(training||(oni!=null&&isOni(p.getUniqueId())));if(oniSkill&&hasOniPassive(OniPassiveSkill.MASTERY))cd=Math.max(1,(int)Math.ceil(cd*Math.max(.50,Math.min(1.0,getConfig().getDouble("oni-passive-skills.mastery-cooldown-multiplier",.85)))));if(oniSkill&&hasOniPassive(OniPassiveSkill.TERRITORY)&&oniNearAliveHeart(p,getConfig().getDouble("oni-passive-skills.territory-radius",8.0)))cd=Math.max(1,(int)Math.ceil(cd*Math.max(.50,Math.min(1.0,getConfig().getDouble("oni-passive-skills.territory-cooldown-multiplier",.80)))));String k=p.getUniqueId()+":"+id;long now=System.currentTimeMillis();long until=cooldowns.getOrDefault(k,0L);if(until>now){msg(p,"あと &e"+((until-now+999)/1000)+"秒 &f待ってください。");return false;}cooldowns.put(k,now+cd*1000L);if(oniSkill)recoverOniSkillMomentum();return true;}
    ...
    private void useOniSkill(Player p,String action){
        ...
        switch(action){
            case "dakko_tp"->{if(p.isSneaking()&&tryDakkoCloneSwap(p))return;if(!ready(p,action,20+heartPenalty))return;Block b=p.getTargetBlockExact(Math.max(6,20-brokenHearts*3));if(b!=null){p.teleport(b.getLocation().add(.5,1,.5).setDirection(p.getLocation().getDirection()));p.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME,p.getLocation().add(0,1,0),18,.35,.6,.35,.04);}}
    ...
    private void useShikkiHuntingLeap(Player p){
        ...
        int cd=shikkiFrenzyActive()?getConfig().getInt("shikki-skills.hunting-leap.frenzy-cooldown-seconds",3):getConfig().getInt("shikki-skills.hunting-leap.cooldown-seconds",6);
        if(!follow&&!ready(p,"shikki_hunting_leap",cd))return;
    ...
    private void watchShikkiLeapLanding(Player p,UUID id){
        ...
        long until=shikkiLeapChainUntil.getOrDefault(id,0L);int max=Math.max(1,getConfig().getInt("shikki-skills.hunting-leap.max-chain",3));
    ...
    @EventHandler public void onDamage(EntityDamageByEntityEvent e){
        ...
        boolean attackerIsOni=(oni!=null&&isOni(rawAttacker.getUniqueId()))||attackerIsBot;
    ...
    private void useDisconnectedOniSignature(Mob bot,LivingEntity target,double distance){
        ...
        if(type==OniType.DAKKO){
            ...
            if(distance<=9){for(LivingEntity q:playerSideTargetsNear(bot,5.5)){dealFixedDamage(q,6.0);Vector kb=q.getLocation().toVector().subtract(bot.getLocation().toVector());if(kb.lengthSquared()>0)q.setVelocity(kb.normalize().multiply(.8).setY(.25));}bot.getWorld().spawnParticle(Particle.SWEEP_ATTACK,bot.getLocation().add(0,1,0),16,1.5,.5,1.5,.02);}else{Vector dir=target.getEyeLocation().toVector().subtract(bot.getEyeLocation().toVector()).normalize();Location at=bot.getEyeLocation().clone();for(int i=0;i<18;i++){at.add(dir.clone().multiply(1.1));bot.getWorld().spawnParticle(Particle.SQUID_INK,at,3,.15,.15,.15,.01);for(LivingEntity q:playerSideTargetsNearLocation(at,1.25))dealFixedDamage(q,1.5);}}cd=14000L;
        }
        disconnectOniProxySkillCycle.put(id,cycle+1);disconnectOniProxySkillReadyAt.put(id,now+cd);
    }
